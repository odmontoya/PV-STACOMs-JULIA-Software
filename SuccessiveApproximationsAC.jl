...
global Vd = node_data.Vk0[2:N,1];
Sd = (node_data.Pk[2:N,1] + im*node_data.Qk[2:N,1])/Sb
# Iterative formula
error = 1e-10;
tmax = 1000;
for t = 1:tmax
   global Vdt = -Zdd*(inv(LinearAlgebra.diagm(conj(Vd)))*conj(Sd) + Yds*Vs)
   if maximum(abs.(abs.(Vd) - abs.(Vdt)))< error
    global Vol = [Vs;Vdt];
    break
   else
    global Vd = Vdt;
...
Sloss = transpose(Vol)*conj!(Ybus*Vol)*Sb